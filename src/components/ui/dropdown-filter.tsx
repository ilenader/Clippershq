...
export function DropdownFilter({ label, options, value, onChange }: DropdownFilterProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 min-w-[180px] max-w-[min(280px,calc(100vw-2rem))] rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] py-1 shadow-[var(--shadow-elevated)]">
          {options.map((opt) => (
            <button
              key={opt.value}
...
export function MultiDropdown({ label, options, values, onChange, allLabel = "All" }: MultiDropdownProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 min-w-[220px] max-w-[min(280px,calc(100vw-2rem))] rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] py-1 shadow-[var(--shadow-elevated)]">
          <button
